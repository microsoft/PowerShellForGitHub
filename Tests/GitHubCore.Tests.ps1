...
Describe 'Testing ConvertTo-SmarterObject behavior' {
    InModuleScope PowerShellForGitHub {
        ...
        Context 'When a known date property has an empty or invalid date string' {
            $original = [PSCustomObject]@{
                'closed_at' = $null
                'committed_at' = '123'
                'completed_at' = $null
        ...
        Context 'When an object has an empty array' {
            ...
            Write-Host (ConvertTo-Json -InputObject $original) # TODO: Debug, Remove
            Write-Host (ConvertTo-Json -InputObject $converted) # TODO: Debug, remove
